    ...
    def __str__(self):
        if self.child_1:
            return self.name + ' (' + str(
                self.weight) + ') [' + self.child_1.name + ', ' + self.child_2.name + ', ' + self.child_3.name + ']'
        else:
            return self.name + ' (' + str(self.weight) + ') [no children]'
...
def find_root(tree_dict):
    ...
    for node in tree_dict:
        if not tree_dict[node].has_parent():
            return tree_dict[node].name
...
while len(stack) > 0:
    ...
    for line in stack:
        ...
        if children[0] in tree and children[1] in tree and children[2] in tree:
            ...
            tree[children[1]].set_parent(name)
            tree[children[2]].set_parent(name)
...
# TODO: this vvv
tree[root].get_total_weight() # This don't work, fix it
